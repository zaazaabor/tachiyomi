...
sealed class FilterWrapper<T>(val filter: Filter<T>) {
  ...
  companion object {
    fun from(filter: Filter<*>): FilterWrapper<*> {
      return when (filter) {
        is Filter.Note -> Note(filter)
        is Filter.Text -> Text(filter)
        is Filter.Check -> Check(filter)
        is Filter.Select -> Select(filter)
        is Filter.Group -> Group(filter)
        is Filter.Sort -> Sort(filter)
      }
    }
  }
}
